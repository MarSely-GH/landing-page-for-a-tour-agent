window.destinations = [
  { title: "Турция", description: "Семейный формат и удобные отели." },
  { title: "Египет", description: "Теплое море и отдых круглый год." },
  { title: "Шри-Ланка", description: "Природа, океан и спокойный темп." },
  { title: "Вьетнам", description: "Экзотика и комфортные курорты." },
  { title: "Таиланд", description: "Разные форматы и бюджеты." },
  { title: "СНГ и Россия", description: "Близко, удобно, без лишней суеты." },
];
